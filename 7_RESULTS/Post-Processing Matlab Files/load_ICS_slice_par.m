function output = load_ICS_slice_par(filename,strides,offsets)
...
fileinfo = h5info(totalfilename);
dataset = fileinfo.Datasets(1);

data=h5read(totalfilename,['/',dataset.Name]);

if(any(isnan(data)))
    disp(['Data contains NaN in ' totalfilename])
end

iStart = double(h5readatt(totalfilename,['/',dataset.Name],'iStart'))';
iToffset = double(h5readatt(totalfilename,['/',dataset.Name],'iToffset'))';
iXoffset = double(h5readatt(totalfilename,['/',dataset.Name],'iXoffset'))';
iYoffset = double(h5readatt(totalfilename,['/',dataset.Name],'iYoffset'))';
rTheta = double(h5readatt(totalfilename,['/',dataset.Name],'rTheta'))';
rStepsize = double(h5readatt(totalfilename,['/',dataset.Name],'rStepsize'))';
iSaveLength = double(h5readatt(totalfilename,['/',dataset.Name],'iSaveLength'))';
if size(iSaveLength,2) == 5 ;iLengthSlice=iSaveLength(end);iSaveLength=iSaveLength(1:end-1);else iLengthSlice = 0; end
iPermute = double(h5readatt(totalfilename,['/',dataset.Name],'iPermute'))';
iNumProc = double(h5readatt(totalfilename,['/',dataset.Name],'iNumProc'))';
if sum(strcmp({dataset.Attributes(:).Name}, 'iSliceIndex')) ; iSliceIndex = double(h5readatt(totalfilename,['/',dataset.Name],'iSliceIndex'))'; ...
else iSliceIndex = 0 ; end
iIsReal = double(h5readatt(totalfilename,['/',dataset.Name],'iIsReal'))';
...
%     dataind = 0;
%     for proc = firstproc+1:iNumProc-1
%        datatot(dataind+(1:length(datatemp{proc})))= datatemp{proc};
%        dataind = dataind + length(datatemp{proc});
%     end
