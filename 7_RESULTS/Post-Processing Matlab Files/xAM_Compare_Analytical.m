...
an.domain.zpar          = domain.par{3}*1E-3;%([1:an.domain.dims(3)] + domain.start(3)-1)*an.domain.dxpar;
...
an.gauss_win                 = 10/an.medium.freq0* an.OSFactor;
...
% an.p_expr                 = 4E5*exp( - A.^2).*an.psin;
an.p_expr                   = 4.5E4*an.psin;
% an.p_expr                   = 4.5E4/an.medium.freq0*exp( - A.^an.power).*( ...
%                             -an.power.*A^(an.power-1)/(an.gauss_win/2).*sin(w*(t-an.gauss_dl) - an.medium.kappa0*sind(an.theta)*x - an.medium.kappa0*cosd(an.theta)*z) + ...
%                                                   w*cos(w*(t-an.gauss_dl) - an.medium.kappa0*sind(an.theta)*x - an.medium.kappa0*cosd(an.theta)*z) + ...
%                             -an.power.*A^(an.power-1)/(an.gauss_win/2).*sin(w*(t-an.gauss_dl) + an.medium.kappa0*sind(an.theta)*x - an.medium.kappa0*cosd(an.theta)*z) + ...
%                                                   w*cos(w*(t-an.gauss_dl) + an.medium.kappa0*sind(an.theta)*x - an.medium.kappa0*cosd(an.theta)*z)) ; % We should split it to 2 cases because there are different time delays 

an.phi_expr               = int(-an.p_expr,t)/an.medium.rho0;
% an.phi_expr               =   -1E5*exp( - A.^2)/an.medium.rho0.*( ...
%                               sin(w*(t-an.gauss_dl) - an.medium.kappa0*sind(an.theta)*x - an.medium.kappa0*cosd(an.theta)*z) + ...
%                               sin(w*(t-an.gauss_dl) + an.medium.kappa0*sind(an.theta)*x - an.medium.kappa0*cosd(an.theta)*z) );
...
an.lagr_pres_expr         = an.medium.rho0/4*(an.lagr_pres_z_expr + an.lagr_pres_x_expr + an.lagr_pres_t_expr);
% 
...
an.len_after  = an.domain.tdimpar  - an.len_window - an.len_before;
...
an.phi                  = double(subs(subs(an.phi_expr         ,  [znorm xnorm], [an.domain.zpar(an.depth), an.domain.xpar(an.lat)]), tnorm, an.domain.tpar));
an.phi_sq               = double(subs(subs(an.phi_sq_expr_simpl,  [znorm xnorm], [an.domain.zpar(an.depth), an.domain.xpar(an.lat)]), tnorm, an.domain.tpar));
an.nabla_x_sq           = double(subs(subs(an.nabla_x_sq_expr  ,  [znorm xnorm], [an.domain.zpar(an.depth), an.domain.xpar(an.lat)]), tnorm, an.domain.tpar));
an.nabla_z_sq           = double(subs(subs(an.nabla_z_sq_expr  ,  [znorm xnorm], [an.domain.zpar(an.depth), an.domain.xpar(an.lat)]), tnorm, an.domain.tpar));
an.nabla_t_sq           = double(subs(subs(an.nabla_t_sq_expr  ,  [znorm xnorm], [an.domain.zpar(an.depth), an.domain.xpar(an.lat)]), tnorm, an.domain.tpar));
an.lagrangian           = double(subs(subs(an.lagr_expr        ,  [znorm xnorm], [an.domain.zpar(an.depth), an.domain.xpar(an.lat)]), tnorm, an.domain.tpar));
...
%% Lagrangian
an.lagrangian           = double(subs(subs(an.lagr_expr.*exp( - A.^an.power)       , [znorm xnorm], [an.domain.zpar(an.depth), an.domain.xpar(an.lat)]), tnorm, an.domain.tpar));
an.lagrangian           = an.lagrangian;%.* an.rect_win';
[var,an] = Compare_Signals(an, an.lagrangian, 0); % Include all variables from an structure, element number, variable of interert, plot (1)/no-plot(0)
an.lagrangian = var;

%% Source Term
an.source_term          = double(subs(subs(an.source_term_expr .*exp( - A.^an.power), [znorm xnorm], [an.domain.zpar(an.depth), an.domain.xpar(an.lat)]), tnorm, an.domain.tpar));
...
GENERATE_MAP(an, 20*log10(abs(sim.p)), sim.title,sim.ylabel_var);
%%                  = 1E-30;
an.OSFactor     = 10;
GENERATE_MAP(an, plin.data, sim.title,sim.ylabel_var);
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ORIGINAL %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ====================================================== Lagrangian ======================================================
% close all;
...
an.lagrangian_source_term                                       = real(double(an.source_term))*(an.medium.dLambda)^2;
sim.Lagr_ST                                                     = sim.lagrangian_source_term ; % Normalization Factor
...
[an.FreqRange,sim.FreqSpect,an.FreqSpect]                       = Frequency(an, sim.Lagr_ST, an.lagrangian_source_term', an.depth, sim.title, sim.ylabel_var);
                                                                  GENERATE_MAP(an, 20*log10(abs(sim.Lagr_ST)), sim.title, sim.ylabel_var);
...
sim.MNL_ST                                                      = sim.mediumnl_source_term; % Normalization Factor
...
[an.FreqRange,sim.FreqSpect,an.FreqSpect]                       = Frequency(an, sim.MNL_ST, an.mediumnl_source_term', an.depth, sim.title, sim.ylabel_var);
                                                                  GENERATE_MAP(an, 20*log10(abs(sim.MNL_ST)), sim.title, sim.ylabel_var);

%% PLOT RANDOM variables of interest
sim.title = ['Pressure Field $\vert$ Local Nonlinearities, ',num2str(an.theta),'$^{\circ}$'];
sim.ylabel_var                                                  = ['Normalized Pressure [dB]'];

an.filename        = [an.file.dirname '/' 'ContrastSrc' int2string_ICS(1) '_y_002' int2string_ICS(an.domain.beam)];
...
an.OSFactor         = 1;
[an.FreqRange,sim.FreqSpect,an.FreqSpect] = Frequency(an,sim.rnd_var_db , -an.rnd_var', an.depth, sim.title, sim.ylabel_var);
GENERATE_MAP(an, 20*log10(abs(sim.rnd_var_db)), sim.title, sim.ylabel_var);
...
length_z = 1;
% an.domain.zpar = an.domain.zpar(61:end) - min(an.domain.zpar(61:end) + an.domain.dxpar);
...
plot_var = squeeze(max(sim_var));
[Zq,Xq] = meshgrid(an.domain.zpar_OS ,an.domain.xpar_OS);

plot_varINTERP = interp2(Z,X,plot_var(:,size(plot_var,2)-length(an.domain.zpar)+1:end),Zq,Xq,'spline');
...
% travel_dist_xwave = an.domain.zpar(an.depth)*cosd(an.theta) + element_x*sind(an.theta); % m
...
% plot_time_frame(an,frame,arr_length - element_x)

tdshift = an_td - an.td;
% syms z x t tnorm
% an_press                    = double(subs(subs(an.source_term_expr           , [z x], [an.domain.zpar(an.depth), an.domain.xpar(an.lat)]), tnorm, an.domain.tpar));
an_press = var;
% an_press = [zeros(1,round(tdshift*2*an.medium.Fnyq*an.medium.freq0*an.OSFactor)) an_press(1:end-round(tdshift*2*an.medium.Fnyq*an.medium.freq0*an.OSFactor))];
