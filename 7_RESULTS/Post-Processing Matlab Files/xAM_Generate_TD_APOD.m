...
%% XWAVE
td_filename = ['../../5_Input_Files/', location,'td_' num2str(i_theta*10) 'deg_x.dat'];
td_x = abs(x_el - arr.W/2 )*sind(i_theta);
td_x = abs(td_x - max(td_x))'*1E-3/c0*1E6; %usec
size_td = size(td_x);
save(td_filename, 'size_td','td_x','-ASCII', '-DOUBLE')

apod_x_filename = ['../../5_Input_Files/', location,'apod_' num2str(i_theta*10) 'deg_x.dat'];
...
%% LEFT
td_left_filename = ['../../5_Input_Files/', location,'td_' num2str(i_theta*10) 'deg_left.dat'];
...
apod_left_filename = ['../../5_Input_Files/', location,'apod_' num2str(i_theta*10) 'deg_left.dat'];
...
%% RIGHT
td_right_filename = ['../../5_Input_Files/', location,'td_' num2str(i_theta*10) 'deg_right.dat'];
...
apod_right_filename = ['../../5_Input_Files/', location,'apod_' num2str(i_theta*10) 'deg_right.dat'];
