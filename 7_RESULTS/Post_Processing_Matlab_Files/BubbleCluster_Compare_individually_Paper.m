...
ax.YLabel.String = 'Pressure [Pa]';
% legendBubble{2} =['Simulation 1st Iter., RRMSE =',num2str(round(RRMS_ErrorG_an(k,m) *1000)/10),'[%]'];
% % legendBubble{1} =['Analytic '];
% legendBubble{1} =['Simulation 0th Iter. '];
...
% legend([m_plot, pr],'Simulation 0th Iter', 'Simulation 1st Iter')
legend([pr,m_plot],'Simulation','Analytic')
ylim([-45 max(Sp_pan(1:domain.tdimpar)-max(Sp_pan))]);
ylabel('normalized amplitude [dB]');
% title('Frequency Spectrum');
box on;
xlim([0 F_s/2/OSFactor]);
