...
%         BubblePresPad = resampleSINC(plin.data(:,72,80)',n_pad);
    ...
    OSFactor = 	1;
    R_rigid = repmat(5E-6,Bubble.N,1); %R_rigid(1) = 0;
...
%         ddpddt = ddpddt.*exp(-1i*omega/medium.c0.*Bubble.LocGlob(:,3)*1E-3);
    ...
    if ( k_end == domain.dimlen(1) && m_end == domain.dimlen(3))
        ...
        if Bubble.N >6
            ...
            p1 =plot (x1+width*[-1 1 1 -1 -1], z1+width*[-1 -1 1 1 -1],'-','Color',[103 0 0 ]/255,'LineWidth',2) ;
            ...
            %6.34, -0.494, 58, 77
            %5.434, 0.3293, 68, 66
            plot(Bubble.LocGlob(:,3),Bubble.LocGlob(:,1), 'x','Color',[102 0 0]/255,'MarkerSize',20,'LineWidth',4)
        end
%         text('Position', [-1.5 5.5],'String', '(a)','FontSize',35,'FontName','Helvetica')
        ...
        if (strcmp(file.saveplot,'yes'))
            ...
            if (isfile(folder)==0) ;mkdir(folder); end
