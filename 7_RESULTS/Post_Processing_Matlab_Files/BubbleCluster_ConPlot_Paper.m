...
%         end
    ...
    legend(legendtxt,'Location', 'southeast','Orientation','horizontal','NumColumns',4)
...
%         pnl_harmi = 20*log10(squeeze(max(abs(pnl.contrastdata))));
        ...
        color = 'white';
        if Bubble.N >6
            rectangle('Position',[domain.min_dim(domain.dimval(3)) domain.min_dim(domain.dimval(2)) domain.max_dim(domain.dimval(3))-domain.min_dim(domain.dimval(3)) domain.max_dim(domain.dimval(2))-domain.min_dim(domain.dimval(2))],'LineStyle','--','EdgeColor',color,'LineWidth',5)
        elseif Bubble.N>=1
...
%         current_path = pwd;
%         filename2 = ['linear_',spl{2},'_slice',int2str(dslice.num),'.png'];
%         foldername2 = ['../',file.savedir,'/'];
%         cd(fullfile(foldername2))
%         if(isfile(filename2));delete(filename2) ; end
%         cd(current_path)
%         movefile('img2.png', [foldername2,filename2])
%         cd ../
%         figure(f3)
%         folder3 = ['../',file.savedir,'/',file.dirname,'_',spl{1},'_',spl{2},'_slice',int2str(dslice.num),'.png'];
%         cd export_fig/
%         export_fig  img3 -painters -q110
%         if(isfile(folder2));delete(folder2) ; end
