...
function [output,BubbleN] = LoadContrastParams(Bubble,file,filename,input, load_operator, fformat, returnN)
if (nargin<5)
    load_operator='no';
    fformat = 'r';
elseif (nargin<6)
...
if (strcmp(load_operator,'yes'))
    for iProcID =0:0
        ...
        if isfile(filename_loc)
            ...
            if (returnN) ; Bubble.N = length(ReadBubbleParameter)/3; input = zeros(2*Bubble.N,3);end
            
            input(1+Bubble.N:2*Bubble.N,:) = reshape(ReadBubbleParameter,size(input,2),Bubble.N)';
