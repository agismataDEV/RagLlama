...
if (Bubble.N <=1E5 && Bubble.N>0)
    ...
    if (Bubble.N<=1E5 )
        ...
        Inc_Press   = 1E5*exp( - ((tpar_N-6/freq)/(3/freq/2)).^2).* sin(w_driving*(tpar_N-6/freq)).*(1+sign(tpar_N))/2;
            ...
            for m = m_init:m_end
                ...
                for i = domain.beamiterations: domain.beamiterations
                    ...
                    for j = 1: i
                        ...
                        %  Spectral derivative - linear scatterer
                        %                             R_rigid         =   1E-4;
                        ...
                        % Spectral derivative - non linear scatterer
                        Amplitude       =   1E-23;
    ...
    if ( k_end == domain.dimlen(1) && m_end == domain.dimlen(3))
        ...
        if (strcmp(file.saveplot,'yes'))
            ...
            folder = ['../',file.savedir,'/',file.dirname,'_Comparison_Slice',int2str(dslice.num),'.png'];
            cd export_fig/
...
ax.YLabel.String = 'Pressure [Pa]';
%             set(ax,'XTickLabel',ax.XTick*1e+6) ; ax.XLabel.String = 'Time [microsec]';
legendBubble{1} =['Simulation'];
legendBubble{2} =['Analytic '];
...
title(['Analytic vs Simulation of MB scattered pressure at (x,y,z) = (',...
    ...
    num2str(round(100*domain.par{domain.dimval(3)}(SliceIndex(3)))/100),') [mm] , iteration ',num2str(i)])
