...
CONTAINS
  ...
  SUBROUTINE BubbleContrastOperator(cSpace)
	...
	if ( BubbleParams%GridPointsPressure == 'around') ALLOCATE(temploc(size(NeighbGridPoints,1)*BubbleParams%N,2))
	...
	if (BubbleParams%GridPointsPressure == 'around' ) then
  ...
  END SUBROUTINE GeneratePSCloud
	...
	elseif (trim(BubbleParams%Distribution) == 'polydisperse') then
				...
				RadFactor= RadFactor/MAXVAL(RadFactor)*(BubbleParams%PDRange(2)-BubbleParams%PDRange(1))+BubbleParams%PDRange(1)
	endif
    if (minval(Domain_Range(1,:) - (/cSpace%iStartX,cSpace%iStartY,cSpace%iStartZ/)*cSpace%dDx*dLambdaMM) < 0.0D0  .OR. &
         maxval(Domain_Range(2,:) - (/cSpace%iDimX,cSpace%iDimY,cSpace%iDimZ/) * cSpace%dDx*dLambdaMM) >= 0.0D0 )then
       write(acTemp,"('Error in GeneratePSCloud, Domain out of boundaries')")
  ...
  SUBROUTINE ProcBubbleCount(cSpace, dBubbleLocationN, tempcon, iProc_NumBubbles, ibDimT)
    ...
    if (iProc_NumBubbles>0) then
       ALLOCATE(dBubbleCon(ibDimT*BubbleParams%N))
       call MPI_FILE_READ(FILE_HANDLE,  dBubbleCon, INT(ibDimT*BubbleParams%N,KIND=8), MPI_DOUBLE_PRECISION, MPI_STATUS_IGNORE, iErr)
    endif
  ...
  SUBROUTINE TrilinInterp_SNDRCV(cSpace, dBubbleLocationN , i_loc, tempcon, temploc, BubbleID, count_neighbs) 
	...
	integer(i8b)				::		NeighbGridPoints(size(temploc,1)/BubbleParams%N,3), p(size(NeighbGridPoints,1)**(1.0D0/3.0D0)), XYZ_i2(size(p,1)**3)
  ...
  SUBROUTINE CalculateCloudPressure(cSpace, dBubbleLocationN, dBubbleContrast)
    ...
    M = PRODUCT(SHAPE(dBubbleContrast))/BubbleParams%N	; N = GridPointsNum; K = BLayer
  ...
  SUBROUTINE MPI_WRITE_CONTRAST(cSpace, dBubbleContrast, MPI_FILENAME)
    ...
    !
    ! =============================================================================
    ibDimT = size(dBubbleContrast,1)/BubbleParams%N
