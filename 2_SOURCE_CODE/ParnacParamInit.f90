...
!read other model parameters
        ...
        read (iImportUNIT, *, IOSTAT=readstatus) i
        if (i == 0) then
            cModelParams.UsePML = .false.
        else
            cModelParams.UsePML = .true.
        end if
        ...
        ! if (cModelParams.PrimarySourceType /= iEI_LOADFIELD) then
            ...
            if (trim(cSourceParams.srcfilename) == 'none') then
                ...
                if (trim(acTemp) == 'gaussian') then
                    ...
                    read (iImportUNIT, *, IOSTAT=readstatus) cSourceParams.Tdelay
                    errstatus = errstatus + readstatus
                else if (trim(acTemp) == 'integrable') then
                    cSourceParams.srcsigntype = iGI_INTEGRABLE
        ...
        if (cModelParams.ContrastSourceType == iCI_SCATTERER) then
            ...
            read (iImportUNIT, *)
            read (iImportUNIT, *)
            read (iImportUNIT, *, IOSTAT=readstatus) cSourceParams%ScCloudNo
            allocate(ScattererParams(cSourceParams%ScCloudNo))  
            do i = 1,cSourceParams%ScCloudNo
                read (iImportUNIT, *)
                read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%Geometry
                call remove_CR(ScattererParams(i)%Geometry)
                read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%ScType
                call remove_CR(ScattererParams(i)%ScType)
                if (trim(ScattererParams(i)%ScType) == 'microbubble') then
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%kappa_s
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%sigma_w
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%sigma_R0
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%gama
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%chi
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%Solver_Method
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%Solver_Normalize
                elseif (trim(ScattererParams(i)%ScType) == 'linear') then
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%rho1
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%c1
                end if

                read (iImportUNIT, *)
                read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%N
                ALLOCATE (ScattererParams(i)%R0(ScattererParams(i)%N))
                read (iImportUNIT, *)
                read (iImportUNIT, *)
                read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%Distribution
                call remove_CR(ScattererParams(i)%Distribution)
                if (trim(ScattererParams(i)%Distribution) == 'monodisperse') then
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%R0(1)
                    ScattererParams(i)%R0(:) = ScattererParams(i)%R0(1)
                elseif (trim(ScattererParams(i)%Distribution) == 'polydisperse') then
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%PDRange
                    ...
                    errstatus = vdrnggamma(VSL_RNG_METHOD_GAMMA_GNORM_ACCURATE, STREAM2, INT(ScattererParams(i)%N), ScattererParams(i)%R0, 2.0D0, 0.0D0, 2.0D0)
                ...
                if (ScattererParams(i)%ClusterSlicesN > 0) then
                    read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%ClusterSliceDim
                end if
                read (iImportUNIT, *)
                read (iImportUNIT, *, IOSTAT=readstatus) ScattererParams(i)%GridPointsPressure
                call remove_CR(ScattererParams(i)%GridPointsPressure)
            enddo
        end if
        read (iImportUNIT, *)
        read (iImportUNIT, *)
...
! =============================================================================
        ...
        if (trim(cSourceParams.srcfilename) == 'none') then
            ...
            if (cModelParams.PrimarySourceType /= iEI_PLANEWAVE .AND. cModelParams.PrimarySourceType /= iEI_POINTSOURCECLOUD) then
                ...
                if (cSourceParams.srcshapetype == iHI_CYLINDRICAL) then
                    ...
                    write (*, '(" Apodfile in X: ",A)') cSourceParams.phaseapodxfilename
                    write (*, '(" Apodfile in Y: ",A)') cSourceParams.phaseapodyfilename
        ...
        if (cModelParams.ContrastSourceType == iCI_SCATTERER) then
            ...
            do i = 1,cSourceParams%ScCloudNo
                ...
                write (*, '("---------------------------------------------------------------------------")')
                write (*, '(" Domain geometry:              ",A)') trim(ScattererParams(i)%Geometry)
                write (*, '(" Type:              ",A)') trim(ScattererParams(i)%ScType)
                if (trim(ScattererParams(i)%ScType) == 'microbubble') then
                    write (*, '(" kappa_s:   ",E10.3)') ScattererParams(i)%kappa_s
                    write (*, '(" sigma_w:   ",E10.3)') ScattererParams(i)%sigma_w
                    write (*, '(" sigma_R0:  ",E10.3)') ScattererParams(i)%sigma_R0
                    write (*, '(" gamma:     ",E10.3)') ScattererParams(i)%gama
                    write (*, '(" chi:       ",E10.3)') ScattererParams(i)%chi
                    write (*, '(" Marmottant Solver:       ",A)') trim(ScattererParams(i)%Solver_Method)
                    write (*, '(" Normalize in:       ",A)') trim(ScattererParams(i)%Solver_Normalize)
                    write (*, '(" ")')
                elseif (trim(ScattererParams(i)%ScType) == 'linear') then
                    write (*, '(" rho1:   ",E10.3)') ScattererParams(i)%rho1
                    write (*, '(" c1:   ",E10.3)') ScattererParams(i)%c1
                end if
                write (*, '("---------------------------------------------------------------------------")')
                write (*, '("Cluster Parameters")')
                write (*, '("---------------------------------------------------------------------------")')
                write (*, '(" ")')
                write (*, '(" Distribution:              ",A)') trim(ScattererParams(i)%Distribution)
                if (trim(ScattererParams(i)%Distribution) == 'monodisperse') then
                    write (*, '(" Radius:                              ",E10.3)') ScattererParams(i)%R0(1)
                elseif (trim(ScattererParams(i)%Distribution) == 'polydisperse') then
                    write (*, '(" Radius Range:             ",E10.3,E10.3)') MINVAL(ScattererParams(i)%R0), MAXVAL(ScattererParams(i)%R0)
                end if
                write (*, '(" No. of Bubbles:       ",I<int(log10(real(ScattererParams(i)%N,dp))+1)>)') ScattererParams(i)%N
...
!srcfilename
        ...
        if (trim(cSourceParams%srcfilename) /= "none" .AND. cModelParams.PrimarySourceType /= iEI_LOADFIELD) then
#ifndef RUNESTIMATOR
...
!
    ...
    SUBROUTINE ScattererInit()
        integer(i8b) :: i 
        do i = 1,cSourceParams%ScCloudNo
            ScattererParams(i)%sBubbleDir = 'Bubbles/'

            ScattererParams(i)%time_norm = 1.0D0
            ScattererParams(i)%rad_norm = 1.0D0

    !------------ Experimental Values
