MODULE ParnacPointSourceCloud
  ...
  SUBROUTINE BubbleContrastOperator(cSpace)
	...
	integer(i8b)				::		p(16), NeighbGridPoints(size(p,1)**3,3) , XYZ_i2(size(p,1)**3), count_neighbs
    real(dp)             		:: 		dGlobXYZ(3), GlobalPos(size(ones,1),3) , xyz_factor(3), diff(size(ones,1),3), Domain_Range(2,3), dF(size(ones,1),3)
    real(dp)             		:: 		dRadius(1), dLambdaMM, dK_c, LinearAmplitude(cSpace%iDimT), Dirac_factor
    integer(i8b)         		:: 		iDimX, iDimY, iDimZ, iDimT, iLx, iLy, iLz, iOmega
    integer(i8b)         		:: 		iBubble,BubbleProc , iIndex, i, j, l, BubbleProcN(size(ones,1))
    ...
    integer(i8b),allocatable	::  	BubbleID(:), tempbuffer(:) , NeighbGPIndex(:), temploc(:,:)
    real(dp),allocatable		::  	dBubbleContrast(:), dBubbleLocationN(:,:) 
    real(dp),allocatable		::		tempcon(:), temp_globloc(:), LocPres(:,:)
    ...
    real(dp) 					::		dDOmega, dFFTFactor, dMultFactor(cSpace%iDimT), dKappa, FFTWtestarray(1)
	
    integer(i8b) 				::		iNumPlanTransform, iNumPlanTransform_inv, iNumPlanTransformXYZ, iNumPlanTransformXYZ_inv
    ...
    if (.NOT. exists_loc) result = MAKEDIRQQ(trim(trim(sOutputDir)//trim(BubbleParams%sBubbleDir)))
    ...
    iDimW   = iDimT/2 + 1
    dLambdaMM = cMediumParams%c0 /cModelParams%freq0;           ! Normalization factor λ in [mm]
    dK_c = pi / cSpace%dDx
	dKappa = two_pi * 2 * cSpace%dFnyq * cModelParams%freq0 / cMediumParams%c0  * dLambdaMM! This is k * λ for the normalization of the spatial dimension

    dDOmega = two_pi * 2.0_dp * cSpace%dFnyq / real(iDimT,dp)
    dMultFactor = (/ (i,i=0,iDimW-1), (i-iDimT, i = iDimW,iDimT-1) /) * dDOmega 
	
    ibDimT   = OS_Factor * cSpace%iDimT ! This should not be changed because ibDimT is Integer

    !Allocate for memory management
    ALLOCATE(BubbleID(BubbleParams%N),temp_globloc(3+cSpace%iDimT))
    ALLOCATE(dBubbleLocationN(BubbleParams%N,3) )

    iMemAllocated=iMemAllocated + (PRODUCT(SHAPE(temp_globloc))  + PRODUCT(SHAPE(dBubbleLocationN)) )*dpS +  ( PRODUCT(SHAPE(BubbleID)) ) * i4bS
    ...
    if (.NOT. exists_loc) then
       write(acTemp,"('Randomly Position the microbubbles in the domain') ");    call PrintToLog(acTemp,2)
        BubbleAsPointScatterer%N = BubbleParams%N ; BubbleAsPointScatterer%R0 = BubbleParams%R0;BubbleAsPointScatterer%ClusterDimsRatio = BubbleParams%ClusterDimsRatio;
		BubbleAsPointScatterer%MinInBetweenDist = BubbleParams%MinInBetweenDist
	...
	ALLOCATE(temploc(size(NeighbGridPoints,1)*BubbleParams%N,2))
    ALLOCATE(LocPres(size(ones,1),(3+iDimT)*i_loc))
    iMemAllocated=iMemAllocated + (PRODUCT(SHAPE(LocPres)) + PRODUCT(SHAPE(tempcon)) ) * dpS 
    NeighbGPIndex = 0
    LocPres=1D-30
    i_loc=0 
	count_neighbs = 0;
    ...
    ! call dfftw_plan_dft_r2c_1d(iNumPlanTransformXYZ 	, INT(1), FFTWtestarray, cFFTWtestarray , fftw_estimate + fftw_unaligned)
    ! call dfftw_plan_dft_c2r_1d(iNumPlanTransformXYZ_inv , INT(1), cFFTWtestarray, FFTWtestarray , fftw_estimate + fftw_unaligned)
    ! call dfftw_plan_dft_1d(iNumPlanTransformXYZ 	, INT(1), cFFTWtestarray, cFFTWtestarray , fftw_forward, fftw_estimate + fftw_unaligned)
    ! call dfftw_plan_dft_1d(iNumPlanTransformXYZ_inv , INT(1), cFFTWtestarray, cFFTWtestarray , fftw_backward, fftw_estimate + fftw_unaligned)
    ! call dfftw_plan_dft_r2c_3d(iNumPlanTransformXYZ 	, INT(1), INT(1), INT(1), FFTWtestarray, cFFTWtestarray , fftw_estimate + fftw_unaligned)
    ! call dfftw_plan_dft_c2r_3d(iNumPlanTransformXYZ_inv , INT(1), INT(1), INT(1), cFFTWtestarray, FFTWtestarray , fftw_estimate + fftw_unaligned)
       ...
       if (XYZIndex>=0 .AND. XYZIndex<cSpace%cGrid%iD0LocN) then
          ...
          if (dRadius(1)>1.0D-30) then
             do i = 1,size(ones,1)
                ...
                if (maxval(temp_globloc(4:iDimT+3)) <1D-20 .OR. maxval(abs(dBubbleLocationN(iBubble,:) - temp_globloc(1:3))) > size(trilin,1)/2*cSpace%dDx) then
                   BubbleProcN(i) = (iBeamIndexN(3)*iDimY*iDimX+iBeamIndexN(2)*iDimX+iBeamIndexN(1))/cSpace%cGrid%iD0LocN
                   call MPI_Recv(temp_globloc, iDimT+3, MPI_DOUBLE_PRECISION, BubbleProcN(i), (iBubble-1)*size(ones,1)+i,MPI_COMM_WORLD,iStat,iErr) ! IMprove with MPI_Irecv	
                endif
				XYZ_i1(i) = iBeamIndexN(3)*iDimY*iDimX+iBeamIndexN(2)*iDimX+iBeamIndexN(1) - cSpace%cGrid%iProcID * cSpace%cGrid%iD0LocN
                ...
                if ( maxval(abs(dBubbleLocationN(iBubble,:) - GlobalPos(i,:))) > size(trilin,1)/2*cSpace%dDx .OR. maxval(RealPressure_i1(i,:))<1D-20 .OR. log_isnan>0) then
		  ...
		  ! ! ! if ( .NOT. ALL(ABS(V_dd_norm)<1E100_dp) .OR. .NOT. ALL(ABS(V_dd_pad)<1E100_dp)) then ! Check Nan , Large values or Infinity
				...
				! ! ! call PrintToLog(acTemp,1) 
				  open (7, file=trim(trim(sOutputDir)//trim(BubbleParams%sBubbleDir)//trim('output_file_pressure')//int2str(cModelParams%iIter)//int2str(cSpace%cGrid%iProcID)//'.txt'),status = 'UNKNOWN', action='write',position='append')				
				  write(7,*) iBubble
				  write(7,*) RealTime, RealPressure,RealTimePad,RealPressurePad
				  close(7) 
				  ! ! open (8, file=trim(trim(sOutputDir)//trim(BubbleParams%sBubbleDir)//trim('v_dd_norm')//int2str(cModelParams%iIter)//int2str(cSpace%cGrid%iProcID)//'.txt'),status = 'UNKNOWN', action='write',position='append')				
          ...
          ! V_dd_norm = 0.0D0;
          ! BubbleParams%R0(iBubble) = 2.4D-6
		  ...
		  ! ! ! do i =1,size(ones,1)
			  ! ! ! ! j = size(ones,1)-i+1;
			  ! ! ! ! Compute the coefficients for dirac expansion .
			  ! ! ! ! Important to mention is that the coefficient comes from the axisymmetric difference that's why we use j here
			  ! ! ! ! Dirac_factor = ABS(PRODUCT( (size(trilin,1)/2*cSpace%dDx - ABS(diff(i,:))) / (size(trilin,1)/2*cSpace%dDx)  ))
			  ! ! ! Dirac_factor = PRODUCT(dSinc(diff(i,:)))
			  ! ! ! RealPressure = tempcon(1+(i_loc-1)*iDimT:i_loc*iDimT) * Dirac_factor
			  
			  ! ! ! if (XYZ_i1(i)>=0 .AND. XYZ_i1(i)<cSpace%cGrid%iD0LocN) then
				  ! ! ! cSpace%cGrid%parD0(1+ XYZ_i1(i)*iDimT : XYZ_i1(i)*iDimT + iDimT) =  &
				  ! ! ! cSpace%cGrid%parD0(1+ XYZ_i1(i)*iDimT : XYZ_i1(i)*iDimT + iDimT) + RealPressure
			  ! ! ! elseif (BubbleProcN(i)>=0) then 
				  ! ! ! call MPI_Send(RealPressure, iDimT, MPI_DOUBLE_PRECISION, BubbleProcN(i), (iBubble-1)*size(ones,1)+i,MPI_COMM_WORLD,iErr)
			  ! ! ! endif
		  ! ! ! enddo
    ...
    enddo
    call dfftw_destroy_plan(pcGrid%cTransforms%iPlanTransform1D )
    call dfftw_destroy_plan(pcGrid%cTransforms%iPlanTransform1D_inv )
	...
	do j =  1,count_neighbs
       ...
       GlobalPos(1,1) = (pcGrid%aiD0Loc(XYZ_i2(i)+1,1) + iBeamOffsetX(cSpace,pcGrid%aiD0Loc(XYZ_i2(i)+1,3))) * cSpace%dDx
       GlobalPos(1,2) = (pcGrid%aiD0Loc(XYZ_i2(i)+1,2) + iBeamOffsetY(cSpace,pcGrid%aiD0Loc(XYZ_i2(i)+1,3))) * cSpace%dDx
       GlobalPos(1,3) = (pcGrid%aiD0Loc(XYZ_i2(i)+1,3) ) * cSpace%dDx 
	   
	   diff(1,:) =  dBubbleLocationN(iBubble,:) - GlobalPos(1,:)
	   Dirac_factor =  PRODUCT(dSinc(dK_c * diff(1,:))) 
	   ! Dirac_factor = ABS(PRODUCT( (size(p,1)/2*cSpace%dDx - ABS(diff(1,:))) / (size(p,1)/2*cSpace%dDx)  ))
	   
	   cSpace%cGrid%parD0(1+ XYZ_i2(i)*iDimT : XYZ_i2(i) * iDimT + iDimT) =  &
	   cSpace%cGrid%parD0(1+ XYZ_i2(i)*iDimT : XYZ_i2(i) * iDimT + iDimT) + dBubbleContrast(1+(iBubble-1)*iDimT:iBubble*iDimT) * Dirac_factor
	enddo 
	DEALLOCATE(temploc)
    ...
    write(acTemp,"('Write the location of bubbles to output file')");    call PrintToLog(acTemp,2)

    if (cSpace%cGrid%iProcID <=0) then

    ! Save each Bubble's Position 
       open (11, file=trim(trim(sOutputDir)//trim(BubbleParams%sBubbleDir)//'Bubble_Location'//int2str(cModelParams%iIter)//int2str(cSpace%cGrid%iProcID)), status='REPLACE')
       do iBubble= 1 ,BubbleParams%N
  ...
  SUBROUTINE GeneratePSCloud(cSpace, PointSourceType, dBubbleLocationN, Domain_Range , cubes)
    ...
    integer(i8b)							::		arBubble_i(BubbleParams%N), arBubble_j(BubbleParams%N)
    integer(i8b)							:: 		iBubble, counter, diff,iBubbleInside
    integer(i8b)							::		LocXYZ(BubbleParams%N) , LocX(BubbleParams%N), LocY(BubbleParams%N), LocZ(BubbleParams%N) 
    real(dp)								::		randpos(BubbleParams%N)
  ...
  SUBROUTINE RNDGeneratePSCloud(cSpace,PointSourceType,dBubbleLocationN, Domain_Range)
    ...
    integer(i8b),allocatable				::  	Bubble_count(:), BubbleID(:)
    ...
    ! =============================================================================
    PointCloud = PointSourceType
    ALLOCATE(Bubble_count(INT(MultFactor*PointCloud%N,8)), BubbleID(INT(MultFactor*PointCloud%N,8)))
    ALLOCATE(randpos(INT(PointCloud%N*MultFactor,8),3) , RadFactor(INT(PointCloud%N*MultFactor)))
    ...
    do iBubble =1,Geom_Series(cSpace%cGrid%iProcID+1)
       ...
       if( ANY(sqrt( (randpos(BubbleIdx,1) - randpos(1:BubbleIdx-1,1) )**2 + &
            ...
            RadFactor(1:BubbleIdx-1)*PointCloud%R0(1:BubbleIdx-1) <0 )) then
    ...
    MaxBubbles = MAXLOC( (/ (i, i =1,sum(Num_counter) )/),1,Bubble_count(1:sum(Num_counter))-(/ (i, i =0,sum(Num_counter)-1) /) < PointCloud%N)
    ...
    if(sum(Num_counter)==0) then
       Bubble_count(MaxBubbles)=PointCloud%N
    ...
    elseif(Bubble_count(MaxBubbles)<=PointCloud%N .AND. MaxBubbles==sum(Num_counter)) then
       ...
       randpos(PointCloud%N+MaxBubbles:S,1) = Domain_Range(1,1)-1
       BubbleID(1:PointCloud%N) = (/ pack( (/ (i , i = 1,Bubble_count(MaxBubbles) ) /), randpos(1:Bubble_count(MaxBubbles),1) >= Domain_Range(1,1) ) /)
       dBubbleLocationN(1:PointCloud%N,:) = randpos( (/ BubbleID(1:PointCloud%N) /),:)
       PointCloud%R0= PointCloud%R0( (/ BubbleID(1:PointCloud%N) /) )
    endif
  ...
  END SUBROUTINE RNDGeneratePSCloud

  SUBROUTINE ProcBubbleCount(cSpace, dBubbleLocationN, tempcon, iProc_NumBubbles, iDimT)
    ...
    type(Space),  intent(in)				::		cSpace
    integer, intent(inout)					::		iProc_NumBubbles, iDimT
    ...
    integer(i4b)         					::  	iErr , BubbleID(size(dBubbleLocationN,1)), FILE_HANDLE, ErrCode
    integer(i8b)							::		iBubble ,XYZIndex,  iBeamIndex(3), FILE_IN, FILE_OUT , length_Bubble
    ...
    ! =============================================================================
       ...
       ! ! ! integer(i8b)			::		count5(BubbleParams%N/cSpace%cGrid%iProcN,275), Bubble_Loc
       ! ! ! integer(i8b)			::		  length(4), i_x,i, diff_z(BubbleParams%N) 
	   ! ! ! integer(i8b),allocatable   :: count51(:,:)
       ! ! ! real(dp)		     	:: 		diff(BubbleParams%N),dLambdaMM
       
       ! ! ! dLambdaMM = (cMediumParams.c0*1.0D3) /cModelParams%freq0;           ! Normalization factor 
    	! ! ! count5=0; 
    	! ! ! count51=0; 
		! ! ! length_Bubble = BubbleParams%N/cSpace%cGrid%iProcN 
		! ! ! if (cSpace%cGrid%iProcID == cSpace%cGrid%iProcN-1) length_Bubble = BubbleParams%N - BubbleParams%N/cSpace%cGrid%iProcN * cSpace%cGrid%iProcID 
		! ! ! ALLOCATE(count51(length_Bubble,1500)) 
		! ! ! diff_z = 0 ;
       ! ! ! do iBubble =  1, length_Bubble
            ! ! ! diff=0;
            ! ! ! Bubble_Loc = iBubble+BubbleParams%N/cSpace%cGrid%iProcN*cSpace%cGrid%iProcID
        ...
        ! ! ! enddo
        ! ! ! open (7, file=trim(trim(sOutputDir)//trim(BubbleParams%sBubbleDir)//trim('Bubble_Count')//int2str(cModelParams%iIter)//int2str(cSpace%cGrid%iProcID)//'.bin'),status = 'UNKNOWN', action='write',position='append')				
		! ! ! write(7,*) count51
		! ! ! close(7) 
		
    !***********************************CALCULATE HOW MANY MICROBUBBLES ARE LOCATED IN EACH PROCESSOR***********************************************	

    MPI_FILENAME = trim(trim(sOutputDir)//trim(BubbleParams%sBubbleDir)//trim('Bubble_Contrast')//int2str(cModelParams%iIter-1)//int2str(0))
    call MPI_FILE_OPEN(MPI_COMM_WORLD, MPI_FILENAME, MPI_MODE_RDWR, MPI_INFO_NULL, FILE_HANDLE, iErr) 

    if (iProc_NumBubbles>0) then
       ALLOCATE(dBubbleCon(iDimT*BubbleParams%N))
       call MPI_FILE_READ(FILE_HANDLE,  dBubbleCon, INT(iDimT*BubbleParams%N,KIND=8), MPI_DOUBLE_PRECISION, MPI_STATUS_IGNORE, iErr)
    endif
  ...
  SUBROUTINE MPI_WRITE_CONTRAST(cSpace, dBubbleContrast, MPI_FILENAME)
    ...
    MPI_BUF_COUNT = ibDimT*BubbleParams%N/cSpace%cGrid%iProcN;
    if (cSpace%cGrid%iProcID==cSpace%cGrid%iProcN-1) MPI_BUF_COUNT = ibDimT*BubbleParams%N - ibDimT*BubbleParams%N/cSpace%cGrid%iProcN*cSpace%cGrid%iProcID

    FILE_IN     = ibDimT*BubbleParams%N/cSpace%cGrid%iProcN*cSpace%cGrid%iProcID
    FILE_OUT    = ibDimT*BubbleParams%N/cSpace%cGrid%iProcN*(cSpace%cGrid%iProcID+1)
    IF (cSpace%cGrid%iProcID==cSpace%cGrid%iProcN-1) FILE_OUT=ibDimT*BubbleParams%N

    FILE_OFFSET = INT(ibDimT*BubbleParams%N/cSpace%cGrid%iProcN*cSpace%cGrid%iProcID * dpS, kind = MPI_OFFSET_KIND)
  ...
  SUBROUTINE PointSourceCloudOperator(cSpace)
    ...
    ! Generate the cluster and save result on slices defined in input file
    call RNDGeneratePSCloud(cSpace,PointSourceCloudParams,dProtonLocationN, Domain_Range)
    
    write(acTemp,"('No. of Protons consisting the cluster: ', I<INT(log10(real(BubbleParams%N,dp)))+1>, ' in a volume of ', F5.3, ' [mL].')") PointSourceCloudParams%N, PRODUCT(maxval(dProtonLocationN,1)-minval(dProtonLocationN,1))*dLambdaMM**3*1.0D-3;    call PrintToLog(acTemp,2);

	if (cSpace%cGrid%iProcID <=0) then

       ! Save each Bubble's Position 
       open (11, file=trim(trim(sOutputDir)//trim(BubbleParams%sBubbleDir)//'PS_Location'//int2str(cSpace%cGrid%iProcID)), status='NEW')
    ...
    call DeltaPulse(cSpace, PointSourceCloudParams%PointSourceAmplitude, INT(2*cModelParams%Fnyq * cSourceParams%Tdelay,8) , dPointSourceContrast , 'bipolar')
  ...
  SUBROUTINE DeltaPulse(cSpace, PointSourceAmplitude, Tindex, dPointSourceContrast,signal_type)
    ...
    !   LOCAL PARAMETERS      
    !
    !   RealTime       		 dp   The discrete time vector
    !   dK_c_t   			 dp   cutoff frequency for time variable
    !   i       		    i8b   Loop variable
    !   iDimT               i8b   The length of the discrete time vector
    !   DistFromPS           dp   Distance from PS
    !
  ...
  SUBROUTINE CalculateCloudMonoEnergeticPressure(cSpace, dPointSourceLocationN, dPointSourceContrast)
    ...
    real(dp),allocatable			::  dSincMult(:,:), dSincMultTotal(:), P_scattered(:,:)
    real(dp),allocatable			::  dGlobBXYZ(:,:) 
    integer, allocatable 			::	GridPoint_OnProc(:,:)

    integer							::	M, N, K, iDimT, BLayer, iDimX, iDimY, iDimZ, i
   ...
   CALL DGEMM('N','N',M,N,1,1.0D0,RESHAPE(dPointSourceContrast,(/M,1/)), M,  RESHAPE(dSincMultTotal * (dK_c/pi)**3,(/1,N/)), 1, 0.0D0, P_scattered, M)	
    ...
    ! call MapVt(cSpace)
    cSpace%cGrid%parD0 = RESHAPE(P_Scattered * (cModelParams%freq0/cMediumParams%c0),(/M*N/)) ! This correction is because they are point sources
