...
CONTAINS
    ...
    SUBROUTINE ConvolutionGreen(cSpaceS, cSpaceD, inplacemode)
        ...
        integer(i8b) ::                                iLi, iLx, iLy, iIndS, iIndS2, iLen, iOmega
        ...
        ! iLen = ceiling(ABS(cModelParams%Sz)/pcSpaceD%dDx-1)      
        ! do iOmega = 0, pcGridD%iD2locN - 1

        !     pcGridD%pacD2(1 + iOmega*pcGridD%iD2IS :1 + iOmega*pcGridD%iD2IS + iLen*pcGridD%iD2ZS)=0.0D0

        !     pcGridD%pacD2(1 + (iOmega+1)*pcGridD%iD2IS - iLen*pcGridD%iD2ZS :1 + (iOmega+1)*pcGridD%iD2IS )=0.0D0
        ! enddo
    ...
    SUBROUTINE GreenXYZBlockFreqT_lossless(cGreen, cSource, dOmega, dKcutoff, iDiffT, iDimT)
        ...
        do iLi = 0, pcGrid%iD2LocSize - 1
            pcGrid.pacD2(1 + iLi) = pcGrid.pacD2(1 + iLi)*dShiftFactor
        end do
