...
!!!!L.D.
    ...
    SUBROUTINE LagrangianDensity_Ali(cSpace)
        ...
        real(dp), allocatable                        :: dTaperSupportWindow(:), dTaperMaxFreqWindow(:), dTaperMaxFreqWindowP(:), dTaperZ(:)
        real(dp)                                     :: dLeftBand, dRightBand, dFFTFactor, dDOmega

        real(dp)                                     :: dTaperX(cSpace%iDimX), dTaperY(cSpace%iDimY), dTaperWidth
        ...
        iDimZ_Wrap = iDimZ + 2*wrap_error_padZ
        ...
        if (cModelParams.UseSupportTapering .EQV. .true.) then
            ...
            iStart = 0
            do iIndex = 0, pcGrid%iD0LocN - 1
                pcGrid%parD0(iStart + 1:iStart + iDimT) = pcGrid%parD0(iStart + 1:iStart + iDimT)*dTaperSupportWindow
                iStart = iStart + pcGrid%iD0IS
            end do
        ...
        if (cModelParams.UseFreqTapering .EQV. .true.) then
            ...
            do iIndex = 0, pcGrid%iD1LocN - 1
                ...
                phi%cGrid%pacD1(iStart + 1) = 0.0D0 ! @ Omega = 0, give the value of 0
        ...
        dTaperZ = dTaperingWindow(cSpace%iDimZ, cSpace%dDx, dRightBand*cSpace%dDx, dLeftBand*cSpace%dDx)

        ALLOCATE (arBuffer(iDimZ_Wrap), arBuffer1square(iDimZ_Wrap), arBuffer2square(iDimZ_Wrap))
        arBuffer = 0.0D0; 
        call dfftw_plan_dft_1d(phi%cGrid%cTransforms%iPlanTransform1D, iDimZ_Wrap, arBuffer, arBuffer, fftw_forward, fftw_estimate )
        call dfftw_plan_dft_1d(phi%cGrid%cTransforms%iPlanTransform1D_inv, iDimZ_Wrap, arBuffer, arBuffer, fftw_backward, FFTW_ESTIMATE)
        ...
        do iLt = 0, phi%cGrid%iD2LocN - 1! This is the loop for time instants stored locally
            ...
            do iLx = 0, iDimX*iDimY - 1
                ...
                pcGrid%pacD2(iStart + 1:iStart + phi%cGrid%iD2IS:phi%cGrid%iD2ZS) = (pcGrid%pacD2(iStart + 1:iStart + phi%cGrid%iD2IS:phi%cGrid%iD2ZS) + &
                                                                                     (arBuffer1square**2 + im*arBuffer2square**2))!*dTaperZ
        ...
        ! Use iDimY_Wrap for Ysymmetry
        call InitSpace(phiSliceMirror, iSI_XYZSLICE, cModelParams%UseYSymmetry, &
                       cSpace%cGrid%iProcN - 1, cSpace%iDimX, iDimY_Wrap, cSpace%iDimZ, &
        ...
        ALLOCATE (dKVectorRealXYZ(iDimX_Wrap*iDimY_Wrap*iDimZ_Wrap))
        do iLz = 0, iDimZ_Wrap - 1
            do iLy = 0, iDimY_Wrap - 1
                do iLx = 0, iDimX_Wrap - 1
                    dKVectorRealXYZ(iLx + iLy*iDimX_Wrap + iLz*iDimX_Wrap*iDimY_Wrap + 1) = &
                        (dKvectorRealX(iLx + 1)**2 + dKvectorRealY(iLy + 1)**2 + dKvectorRealZ(iLz + 1)**2)
                end do
            end do
        end do
        ...
        pcGrid%pacD1 = pcGrid%pacD1 + phi%cGrid%pacD1*real(cMediumParams%c0**2, dp)  ! Addition of the two terms in the lagrangian ( Kinetic and potential)
        call DestructSpace(phi)
        call ReorderDistr1ToDistr2(pcGrid)

        ! Comments for next time
        ! Why there is a difference result for 2nd spatial derivative of Lagrangian density between FD and FFT
        ! Why is there a difference between the spatial and temporal derivative when they are expected to be very close?
        ! (Even though amplitude wise the previous results are the same for the 2 proceedures)
        ...
        do iLt = 0, phi%cGrid%iD2LocN - 1! This is the loop for time instants stored locally
            ...
            do iLz = 0, iDimZ - 1
                do iLy = 0, iDimY - 1
                    do iLx = 0, iDimX - 1
                        iStart = 0 + iLx + iLy*iDimX + iLz*iDimX*iDimY + Timestart  ! This is the index for all the possible x,y for the same z
                        pcGrid%pacD2(iStart + 1) = pcGrid%pacD2(iStart + 1) * dTaperZ(1+iLz) * dTaperY(1+iLy) * dTaperX(1+iLx)
                    end do
                end do
            end do
        end do
        ...
        ! do i = 1, cModelParams%numslices
        !     if ((cModelParams%xyzslicebeam(i) == 0) .or. (cModelParams%xyzslicebeam(i) == -1)) then
        !         filename = trim(trim(sOutputDir)//trim('LagrangianPressureTemporal')//int2str(cModelParams%iIter))//'_'//cModelParams%xyzslicedim(i)// &
        !                    int2str(i)//int2str(0)
        ...
        ! do i = 1, cModelParams%numslices
        !     if ((cModelParams%xyzslicebeam(i) == 0) .or. (cModelParams%xyzslicebeam(i) == -1)) then
        !         filename = trim(trim(sOutputDir)//trim('LagrangianPressureSpatial')//int2str(cModelParams%iIter))//'_'//cModelParams%xyzslicedim(i)// &
        !                    int2str(i)//int2str(0)
    ...
    SUBROUTINE SpatialDerivative_w_wo_Ali(cSpace, cDerivLookup, arBuffer, dKVectorXYZ, Alias, Order)
        ...
        real(dp)                                     :: dFFTFactor
        integer(i8b)                                 :: iLenXYZ, iStart, iLx, iIndex, wrap_error_padZ, iDimWrapZ
        ...
        if (Alias) then
            ...
            if (Order == 1) then
                ...
                do iIndex = 0, cSpace%cGrid%iD2YL*cSpace%cGrid%iD2ZL - 1
                    ...
                    tempbuffer = tempbufferXYZ(iStart + 1:iStart + cSpace%cGrid%iD2XL)
                ...
                wrap_error_padZ = 0; 
                iDimWrapZ = cSpace%cGrid%iD2ZL + 2*wrap_error_padZ
                ALLOCATE (tempbuffer(iDimWrapZ))
                tempbuffer = 0.0D0
                do iLx = 0, cSpace%cGrid%iD2XL*cSpace%cGrid%iD2YL - 1

                    iStart = iLx
                    tempbuffer(wrap_error_padZ + 1:iDimWrapZ - wrap_error_padZ) = tempbufferXYZ(iStart + 1:iStart + cSpace%cGrid%iD2IS:cSpace%cGrid%iD2ZS)
                    call DerivativeComplex(tempbuffer/cMediumParams%c0, tempbuffer, iDimWrapZ, cSpace%dDx, cDerivLookup%arWeights, cDerivLookup%aiPoints)
                    call DerivativeComplex(tempbuffer/cMediumParams%c0, tempbuffer, iDimWrapZ, cSpace%dDx, cDerivLookup%arWeights, cDerivLookup%aiPoints)

                    cSpace%cGrid%pacD2(iStart + 1:iStart + cSpace%cGrid%iD2IS:cSpace%cGrid%iD2ZS) = &
                        cSpace%cGrid%pacD2(iStart + 1:iStart + cSpace%cGrid%iD2IS:cSpace%cGrid%iD2ZS) + tempbuffer(wrap_error_padZ + 1:iDimWrapZ - wrap_error_padZ)
    ...
    SUBROUTINE LagrangianDensity_Simpl_Ali(cSpace)
        ...
        call PrintToLog("Compute the Laplacian Operator of phi^2", 2)
        ...
        call PrintToLog("Compute the time derivative of phi^2", 2)
        ...
        if (cModelParams.UseFreqTapering .EQV. .true.) then
            ...
            do iIndex = 0, pcGrid%iD1LocN - 1
                ...
                pcGrid%pacD1(iStart + 1:iStart + iDimT+1) = &
                    pcGrid%pacD1(iStart + 1:iStart + iDimT+1)*dTaperingWindow(iDimT+1, 1.0_dp/(2*iDimT*cSpace%dDt), 1.0D0, 0.1D0)*dFFTFactor* &
                    (-((/(i, i=0, iDimW - 1) , (i - iDimT+1, i=iDimW, iDimT) /) *dDOmega)**2)
                phi%cGrid%pacD1(iStart + 1:iStart + iDimT+1) = &
                    phi%cGrid%pacD1(iStart + 1:iStart + iDimT+1)*dTaperingWindow(iDimT+1, 1.0_dp/(2*iDimT*cSpace%dDt), 1.0D0, 0.1D0)*dFFTFactor
                phi%cGrid%pacD1(iStart + 1) = 0.0D0 ! @ Omega = 0, give the value of 0
