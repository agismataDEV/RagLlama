...
CONTAINS
    ...
    SUBROUTINE MapVRealComplex(acData, acWrap, iDim, aiBlockL, aiBlockS, iWrapL, aiWrapS, aiOffsets)
        ...
        do iLi = 0, iIndMax - 1
            ...
            if (mod(aiBlockL(1), 2_i8b) == 1) then
                arBufferIn(1:aiBlockL(1):2) = dreal(acData(iIndData:iIndData + ((aiBlockL(1) + 1)/2 - 1)*aiBlockS(1):aiBlockS(1))); 
            else
                arBufferIn(1:aiBlockL(1):2) = dreal(acData(iIndData:iIndData + (aiBlockL(1)/2 - 1)*aiBlockS(1):aiBlockS(1))); 
            end if
    ...
    SUBROUTINE MapVInvRealComplex(acData, acWrap, iDim, aiBlockL, aiBlockS, iWrapL, aiWrapS, aiOffsets)
        ...
        do iLi = 0, iIndMax - 1
            ...
            !fill real input buffer from complex array
            if (mod(iWrapL, 2_i8b) == 1) then
                arBufferIn(1:iWrapL:2) = dreal(acWrap(iIndWrap:iIndWrap + ((iWrapL + 1)/2 - 1)*aiWrapS(1):aiWrapS(1))); 
            else
                arBufferIn(1:iWrapL:2) = dreal(acWrap(iIndWrap:iIndWrap + (iWrapL/2 - 1)*aiWrapS(1):aiWrapS(1))); 
            end if
