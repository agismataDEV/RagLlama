...
iStart=double(attributes(1).Value);
iToffset=double(attributes(2).Value);
iXoffset=double(attributes(3).Value);
iYoffset=double(attributes(4).Value);
rTheta=double(attributes(5).Value);
rStepsize=double(attributes(6).Value);
iSavelength=double(attributes(7).Value);
iPermute=double(attributes(8).Value);
iNumproc=double(attributes(9).Value);
bIsreal=double(attributes(10).Value);
...
if (all(strides==[1 1 1 1])&&all(offsets==[0 0 0 0]))
    ...
    if ((any(iPermute~=[1 2 3 4])||bIsreal~=1))
